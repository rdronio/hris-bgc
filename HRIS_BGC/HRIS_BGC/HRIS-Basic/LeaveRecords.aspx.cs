...
namespace HRIS_Basic
{
    public partial class LeaveRecords : System.Web.UI.Page
    {
        ...
        protected void btnRequest_Click(object sender, EventArgs e)
        {
            ...
            if (department_id == "0")
            {
                Response.Write("<script>confirm('No department selected.');</script>");
                ShowModal();
                return;
            }

            string empid = drpEmployee.SelectedValue;
            string employee_name = drpEmployee.SelectedItem.Text;
            string status = "1";
            string leave = "";
            ...
            if (empid == "0")
            {
                Response.Write("<script>confirm('No employee selected.');</script>");
                ShowModal();
                return;
            }
            ...
            {
                Response.Write("<script>confirm('Leave credit for "+ employee_name +" is not available.');</script>");
                ShowModal();
                return;
            ...
            objLeave.FileLeave(int.Parse(empid), leave, fromDate, toDate, reason, status, numberOfDays, allDates);
